...
- If a paired Windows node receives a new DHCP hostname or IP, keep its existing identity and run `mirabridge node reconnect NODE_ID --host NEW_HOST` on the Mac. The command must match the live key to the already pinned fingerprint, complete a Worker handshake, and roll back on failure. Never delete `known_hosts` or accept a different key merely to recover connectivity.
